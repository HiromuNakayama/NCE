...
############################################################
# Experiment 4: transition-regime difficulty
############################################################
...
K <- 2
T_len <- 3000
B <- as.integer(Sys.getenv("NCE_REGIME_B", unset = "100"))
R_fit <- as.integer(Sys.getenv("NCE_REGIME_R", unset = "50"))
...
run_one_regime_experiment <- function(
  A_true,
  regime_name,
  T_len,
  K,
  ...
  A_frobenius_error <- norm(fit$A - A_true, type = "F")

  gamma_L1_to_oracle <- l1_gamma_distance(
    gamma_hat = fit$gamma,
    gamma_ref = gamma_oracle
  )

  n_switches <- sum(z_true[2:T_len] != z_true[1:(T_len - 1)])
  switch_rate <- n_switches / (T_len - 1)

  data.frame(
    regime = regime_name,
    A_frobenius_error = A_frobenius_error,
    accuracy = accuracy,
    gamma_L1_to_oracle = gamma_L1_to_oracle,
...
results_list <- vector("list", length(A_regimes) * B)
idx <- 1

for (regime_name in names(A_regimes)) {
  A_true <- A_regimes[[regime_name]]

  for (b in 1:B) {
    cat("regime:", regime_name, " replication:", b, "\n")

    results_list[[idx]] <- run_one_regime_experiment(
      A_true = A_true,
...
results_regime <- do.call(rbind, results_list)

############################################################
# 5. Mean and SD summaries
############################################################
...
summary_table <- data.frame(
  regime = summary_mean$regime,
  A_frobenius_error = format_mean_sd(
    summary_mean$A_frobenius_error,
    summary_sd$A_frobenius_error
  ),
  accuracy = format_mean_sd(
    summary_mean$accuracy,
    summary_sd$accuracy
  ),
  gamma_L1_to_oracle = format_mean_sd(
    summary_mean$gamma_L1_to_oracle,
    summary_sd$gamma_L1_to_oracle
  ),
  switch_rate = format_mean_sd(
    summary_mean$switch_rate,
    summary_sd$switch_rate
  )
)
...
boxplot(
  A_frobenius_error ~ regime,
  data = results_regime,
  main = expression("Error of " * hat(A)),
  ylab = expression("||" * hat(A) - A^"*" * "||"[F])
)

boxplot(
  accuracy ~ regime,
  data = results_regime,
  main = "State classification accuracy",
  ylab = "accuracy"
)

boxplot(
  gamma_L1_to_oracle ~ regime,
  data = results_regime,
  main = expression(gamma * " distance to oracle"),
  ylab = expression(
    T^{-1} * sum[t] * sum[k] *
      "|" * hat(gamma)[t](k) - gamma[t]^oracle(k) * "|"
  )
)

############################################################
# 8. Optional typical examples
############################################################

show_typical_examples <- identical(
  Sys.getenv("NCE_SHOW_TYPICAL_EXAMPLES", unset = "false"),
  "true"
)

if (show_typical_examples) {
  par(mfrow = c(3, 2))
  ...
  for (regime_name in names(A_regimes)) {
    ...
    plot(
      z_true,
      type = "s",
      main = paste(regime_name, ": true states"),
      xlab = "t",
      ylab = expression(z[t])
    )
    ...
    plot(
      ...
      type = "l",
      ylim = c(0, 1),
      main = paste(regime_name, ": smoothing prob. state 1"),
      xlab = "t",
