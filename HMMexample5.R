set.seed(123)

############################################################
# 0. Basic settings
############################################################

K <- 2
T_len <- 5000

A_true <- matrix(
  c(0.98, 0.02,
    0.03, 0.97),
  nrow = K,
  byrow = TRUE
)

mu_true <- c(-0.5, 0.5)
lambda_true <- c(0.7, 1.8)

############################################################
# 1. Stationary distribution
############################################################

stationary_dist <- function(A) {
  K <- nrow(A)

  M <- t(A) - diag(K)
  M[K, ] <- rep(1, K)

  b <- c(rep(0, K - 1), 1)
  rho <- solve(M, b)

  rho <- pmax(rho, 0)
  rho / sum(rho)
}

rho_true <- stationary_dist(A_true)

c_true <- -0.5 * log(2 * pi) + 0.5 * log(lambda_true)

############################################################
# 2. HMM data generation
############################################################

simulate_hmm <- function(T_len, A, rho, mu, lambda) {
  K <- length(rho)

  z <- integer(T_len)
  x <- numeric(T_len)

  z[1] <- sample(1:K, size = 1, prob = rho)
  x[1] <- rnorm(
    1,
    mean = mu[z[1]],
    sd = 1 / sqrt(lambda[z[1]])
  )

  for (t in 2:T_len) {
    z[t] <- sample(1:K, size = 1, prob = A[z[t - 1], ])
    x[t] <- rnorm(
      1,
      mean = mu[z[t]],
      sd = 1 / sqrt(lambda[z[t]])
    )
  }

  list(x = x, z = z)
}

dat <- simulate_hmm(
  T_len = T_len,
  A = A_true,
  rho = rho_true,
  mu = mu_true,
  lambda = lambda_true
)

x <- dat$x
z_true <- dat$z

############################################################
# 3. Unnormalized observation model
############################################################

log_tilde_p <- function(x, mu, log_lambda) {
  lambda <- exp(log_lambda)
  -0.5 * lambda * (x - mu)^2
}

score_fun <- function(x, mu, log_lambda, c) {
  log_tilde_p(x, mu, log_lambda) + c
}

############################################################
# 4. HMM filtering and smoothing
############################################################

filter_smooth_with_xi <- function(x, A, rho, mu, log_lambda, c) {
  T_len <- length(x)
  K <- length(rho)

  log_b <- matrix(0, nrow = T_len, ncol = K)

  for (k in 1:K) {
    log_b[, k] <- score_fun(
      x = x,
      mu = mu[k],
      log_lambda = log_lambda[k],
      c = c[k]
    )
  }

  log_b_shift <- log_b - apply(log_b, 1, max)
  b <- exp(log_b_shift)

  alpha <- matrix(0, nrow = T_len, ncol = K)

  alpha[1, ] <- rho * b[1, ]
  alpha[1, ] <- alpha[1, ] / sum(alpha[1, ])

  for (t in 2:T_len) {
    pred <- as.numeric(alpha[t - 1, ] %*% A)
    alpha[t, ] <- b[t, ] * pred
    alpha[t, ] <- alpha[t, ] / sum(alpha[t, ])
  }

  beta <- matrix(0, nrow = T_len, ncol = K)
  beta[T_len, ] <- rep(1, K)

  for (t in (T_len - 1):1) {
    for (j in 1:K) {
      beta[t, j] <- sum(
        A[j, ] * b[t + 1, ] * beta[t + 1, ]
      )
    }
    beta[t, ] <- beta[t, ] / sum(beta[t, ])
  }

  gamma <- alpha * beta
  gamma <- gamma / rowSums(gamma)

  xi <- array(0, dim = c(T_len, K, K))

  for (t in 2:T_len) {
    numer <- matrix(0, nrow = K, ncol = K)

    for (j in 1:K) {
      for (k in 1:K) {
        numer[j, k] <-
          alpha[t - 1, j] *
          A[j, k] *
          b[t, k] *
          beta[t, k]
      }
    }

    xi[t, , ] <- numer / sum(numer)
  }

  list(
    gamma = gamma,
    xi = xi,
    alpha = alpha,
    beta = beta
  )
}

############################################################
# 5. Weighted NCE common functions
############################################################

weighted_nce_objective <- function(
  par,
  x,
  weight_k,
  noise_y,
  noise_density_fun
) {
  mu <- par[1]
  log_lambda <- par[2]
  c <- par[3]

  Nk <- sum(weight_k)
  Mk <- length(noise_y)

  sx <- score_fun(x, mu, log_lambda, c)
  sy <- score_fun(noise_y, mu, log_lambda, c)

  nx <- noise_density_fun(x)
  ny <- noise_density_fun(noise_y)

  log_num_x <- log(Nk) + sx
  log_noise_x <- log(Mk) + log(nx)

  max_x <- pmax(log_num_x, log_noise_x)
  log_den_x <- max_x +
    log(exp(log_num_x - max_x) + exp(log_noise_x - max_x))

  log_D_x <- log_num_x - log_den_x

  log_num_y <- log(Nk) + sy
  log_noise_y <- log(Mk) + log(ny)

  max_y <- pmax(log_num_y, log_noise_y)
  log_den_y <- max_y +
    log(exp(log_num_y - max_y) + exp(log_noise_y - max_y))

  log_1m_D_y <- log_noise_y - log_den_y

  obj <- sum(weight_k * log_D_x) + sum(log_1m_D_y)

  -obj
}

update_one_state_nce <- function(
  x,
  weight_k,
  init_par,
  noise_y,
  noise_density_fun
) {
  fit <- optim(
    par = init_par,
    fn = weighted_nce_objective,
    x = x,
    weight_k = weight_k,
    noise_y = noise_y,
    noise_density_fun = noise_density_fun,
    method = "BFGS",
    control = list(maxit = 300, reltol = 1e-8)
  )

  fit$par
}

############################################################
# 6. Weighted NCE-HMM
############################################################

initialize_A_from_labels <- function(labels, K, pseudocount = 1) {
  counts <- matrix(pseudocount, nrow = K, ncol = K)

  for (t in 2:length(labels)) {
    counts[labels[t - 1], labels[t]] <-
      counts[labels[t - 1], labels[t]] + 1
  }

  counts / rowSums(counts)
}

fit_weighted_nce_hmm <- function(
  x,
  K,
  R = 50,
  noise_ratio = 5,
  damping = 0.7,
  tol = 1e-6
) {
  T_len <- length(x)

  km <- kmeans(x, centers = K, nstart = 30)
  cl <- km$cluster

  mu <- numeric(K)
  log_lambda <- numeric(K)
  c <- rep(0, K)

  for (k in 1:K) {
    xk <- x[cl == k]
    if (length(xk) < 5) {
      xk <- x
    }

    mu[k] <- mean(xk)
    log_lambda[k] <- log(1 / var(xk))
  }

  ord <- order(mu)
  mu <- mu[ord]
  log_lambda <- log_lambda[ord]
  c <- c[ord]

  new_label <- integer(K)
  new_label[ord] <- 1:K
  cl <- new_label[cl]

  A <- initialize_A_from_labels(cl, K)

  noise_mean <- mean(x)
  noise_sd <- sd(x) * 2

  noise_density_fun <- function(u) {
    dnorm(u, mean = noise_mean, sd = noise_sd)
  }

  M_fixed <- max(500, round(noise_ratio * T_len / K))

  noise_list <- vector("list", K)
  for (k in 1:K) {
    noise_list[[k]] <- rnorm(
      M_fixed,
      mean = noise_mean,
      sd = noise_sd
    )
  }

  history <- data.frame()

  for (r in 1:R) {
    old_mu <- mu
    old_log_lambda <- log_lambda
    old_c <- c
    old_A <- A

    rho <- stationary_dist(A)

    fit_state <- filter_smooth_with_xi(
      x = x,
      A = A,
      rho = rho,
      mu = mu,
      log_lambda = log_lambda,
      c = c
    )

    gamma <- fit_state$gamma
    xi <- fit_state$xi

    A_new <- matrix(0, nrow = K, ncol = K)

    for (j in 1:K) {
      denom <- sum(gamma[1:(T_len - 1), j])

      for (k in 1:K) {
        numer <- sum(xi[2:T_len, j, k])
        A_new[j, k] <- numer / denom
      }
    }

    A_new <- A_new / rowSums(A_new)

    A <- (1 - damping) * A + damping * A_new
    A <- A / rowSums(A)

    for (k in 1:K) {
      init_par <- c(mu[k], log_lambda[k], c[k])

      new_par <- update_one_state_nce(
        x = x,
        weight_k = gamma[, k],
        init_par = init_par,
        noise_y = noise_list[[k]],
        noise_density_fun = noise_density_fun
      )

      mu[k] <- (1 - damping) * mu[k] + damping * new_par[1]
      log_lambda[k] <-
        (1 - damping) * log_lambda[k] + damping * new_par[2]
      c[k] <- (1 - damping) * c[k] + damping * new_par[3]
    }

    ord <- order(mu)
    mu <- mu[ord]
    log_lambda <- log_lambda[ord]
    c <- c[ord]
    A <- A[ord, ord]

    diff <- sum(abs(mu - old_mu)) +
      sum(abs(log_lambda - old_log_lambda)) +
      sum(abs(c - old_c)) +
      sum(abs(A - old_A))

    history <- rbind(
      history,
      data.frame(
        iter = r,
        diff = diff,
        mu1 = mu[1],
        mu2 = mu[2],
        lambda1 = exp(log_lambda[1]),
        lambda2 = exp(log_lambda[2]),
        c1 = c[1],
        c2 = c[2],
        A11 = A[1, 1],
        A12 = A[1, 2],
        A21 = A[2, 1],
        A22 = A[2, 2]
      )
    )

    if (diff < tol) {
      break
    }
  }

  rho_final <- stationary_dist(A)

  fit_final <- filter_smooth_with_xi(
    x = x,
    A = A,
    rho = rho_final,
    mu = mu,
    log_lambda = log_lambda,
    c = c
  )

  list(
    mu = mu,
    lambda = exp(log_lambda),
    log_lambda = log_lambda,
    c = c,
    A = A,
    rho = rho_final,
    gamma = fit_final$gamma,
    history = history
  )
}

############################################################
# 7. Weighted NCE-mixture
############################################################

softmax_rows <- function(log_mat) {
  log_mat_shift <- log_mat - apply(log_mat, 1, max)
  mat <- exp(log_mat_shift)
  mat / rowSums(mat)
}

fit_weighted_nce_mixture <- function(
  x,
  K,
  R = 50,
  noise_ratio = 5,
  damping = 0.7,
  tol = 1e-6
) {
  T_len <- length(x)

  km <- kmeans(x, centers = K, nstart = 30)
  cl <- km$cluster

  mu <- numeric(K)
  log_lambda <- numeric(K)
  c <- rep(0, K)
  pi <- numeric(K)

  for (k in 1:K) {
    xk <- x[cl == k]
    if (length(xk) < 5) {
      xk <- x
    }

    mu[k] <- mean(xk)
    log_lambda[k] <- log(1 / var(xk))
    pi[k] <- mean(cl == k)
  }

  ord <- order(mu)
  mu <- mu[ord]
  log_lambda <- log_lambda[ord]
  c <- c[ord]
  pi <- pi[ord]

  noise_mean <- mean(x)
  noise_sd <- sd(x) * 2

  noise_density_fun <- function(u) {
    dnorm(u, mean = noise_mean, sd = noise_sd)
  }

  M_fixed <- max(500, round(noise_ratio * T_len / K))

  noise_list <- vector("list", K)
  for (k in 1:K) {
    noise_list[[k]] <- rnorm(
      M_fixed,
      mean = noise_mean,
      sd = noise_sd
    )
  }

  history <- data.frame()

  for (r in 1:R) {
    old_mu <- mu
    old_log_lambda <- log_lambda
    old_c <- c
    old_pi <- pi

    log_weight <- matrix(0, nrow = T_len, ncol = K)

    for (k in 1:K) {
      log_weight[, k] <-
        log(pi[k]) +
        score_fun(
          x = x,
          mu = mu[k],
          log_lambda = log_lambda[k],
          c = c[k]
        )
    }

    resp <- softmax_rows(log_weight)

    pi_new <- colMeans(resp)
    pi <- (1 - damping) * pi + damping * pi_new
    pi <- pi / sum(pi)

    for (k in 1:K) {
      init_par <- c(mu[k], log_lambda[k], c[k])

      new_par <- update_one_state_nce(
        x = x,
        weight_k = resp[, k],
        init_par = init_par,
        noise_y = noise_list[[k]],
        noise_density_fun = noise_density_fun
      )

      mu[k] <- (1 - damping) * mu[k] + damping * new_par[1]
      log_lambda[k] <-
        (1 - damping) * log_lambda[k] + damping * new_par[2]
      c[k] <- (1 - damping) * c[k] + damping * new_par[3]
    }

    ord <- order(mu)
    mu <- mu[ord]
    log_lambda <- log_lambda[ord]
    c <- c[ord]
    pi <- pi[ord]

    diff <- sum(abs(mu - old_mu)) +
      sum(abs(log_lambda - old_log_lambda)) +
      sum(abs(c - old_c)) +
      sum(abs(pi - old_pi))

    history <- rbind(
      history,
      data.frame(
        iter = r,
        diff = diff,
        mu1 = mu[1],
        mu2 = mu[2],
        lambda1 = exp(log_lambda[1]),
        lambda2 = exp(log_lambda[2]),
        c1 = c[1],
        c2 = c[2],
        pi1 = pi[1],
        pi2 = pi[2]
      )
    )

    if (diff < tol) {
      break
    }
  }

  log_weight <- matrix(0, nrow = T_len, ncol = K)

  for (k in 1:K) {
    log_weight[, k] <-
      log(pi[k]) +
      score_fun(
        x = x,
        mu = mu[k],
        log_lambda = log_lambda[k],
        c = c[k]
      )
  }

  resp_final <- softmax_rows(log_weight)

  list(
    mu = mu,
    lambda = exp(log_lambda),
    log_lambda = log_lambda,
    c = c,
    pi = pi,
    resp = resp_final,
    history = history
  )
}

############################################################
# 8. Estimation
############################################################

fit_hmm <- fit_weighted_nce_hmm(
  x = x,
  K = K,
  R = 50,
  noise_ratio = 5,
  damping = 0.7
)

fit_mix <- fit_weighted_nce_mixture(
  x = x,
  K = K,
  R = 50,
  noise_ratio = 5,
  damping = 0.7
)

############################################################
# 9. Oracle HMM and Naive HMM
############################################################

fit_oracle_hmm <- filter_smooth_with_xi(
  x = x,
  A = A_true,
  rho = rho_true,
  mu = mu_true,
  log_lambda = log(lambda_true),
  c = c_true
)

gamma_oracle_hmm <- fit_oracle_hmm$gamma

fit_naive_hmm <- filter_smooth_with_xi(
  x = x,
  A = fit_hmm$A,
  rho = fit_hmm$rho,
  mu = fit_hmm$mu,
  log_lambda = log(fit_hmm$lambda),
  c = rep(0, K)
)

gamma_naive_hmm <- fit_naive_hmm$gamma

############################################################
# 10. Evaluation functions
############################################################

best_accuracy_2state <- function(z_hat, z_true) {
  acc1 <- mean(z_hat == z_true)
  acc2 <- mean((3 - z_hat) == z_true)
  max(acc1, acc2)
}

l1_gamma_distance <- function(gamma_hat, gamma_ref) {
  mean(rowSums(abs(gamma_hat - gamma_ref)))
}

mean_entropy <- function(prob_mat) {
  eps <- 1e-12
  mean(-rowSums(prob_mat * log(prob_mat + eps)))
}

############################################################
# 11. Comparison
############################################################

z_hat_oracle_hmm <- apply(gamma_oracle_hmm, 1, which.max)
z_hat_hmm <- apply(fit_hmm$gamma, 1, which.max)
z_hat_mix <- apply(fit_mix$resp, 1, which.max)
z_hat_naive_hmm <- apply(gamma_naive_hmm, 1, which.max)

comparison <- data.frame(
  method = c(
    "Oracle HMM",
    "Weighted NCE-HMM",
    "Weighted NCE-mixture",
    "Naive HMM"
  ),
  accuracy = c(
    best_accuracy_2state(z_hat_oracle_hmm, z_true),
    best_accuracy_2state(z_hat_hmm, z_true),
    best_accuracy_2state(z_hat_mix, z_true),
    best_accuracy_2state(z_hat_naive_hmm, z_true)
  ),
  gamma_L1_to_oracle_HMM = c(
    0,
    l1_gamma_distance(fit_hmm$gamma, gamma_oracle_hmm),
    l1_gamma_distance(fit_mix$resp, gamma_oracle_hmm),
    l1_gamma_distance(gamma_naive_hmm, gamma_oracle_hmm)
  ),
  mean_entropy = c(
    mean_entropy(gamma_oracle_hmm),
    mean_entropy(fit_hmm$gamma),
    mean_entropy(fit_mix$resp),
    mean_entropy(gamma_naive_hmm)
  )
)

print(comparison)

############################################################
# 12. Estimated parameters
############################################################

cat("True A:\n")
print(A_true)

cat("Estimated A by Weighted NCE-HMM:\n")
print(fit_hmm$A)

cat("True stationary rho:\n")
print(rho_true)

cat("Estimated stationary rho by Weighted NCE-HMM:\n")
print(fit_hmm$rho)

cat("True mu:\n")
print(mu_true)

cat("Estimated mu by Weighted NCE-HMM:\n")
print(fit_hmm$mu)

cat("Estimated mu by Weighted NCE-mixture:\n")
print(fit_mix$mu)

cat("True lambda:\n")
print(lambda_true)

cat("Estimated lambda by Weighted NCE-HMM:\n")
print(fit_hmm$lambda)

cat("Estimated lambda by Weighted NCE-mixture:\n")
print(fit_mix$lambda)

cat("True c:\n")
print(c_true)

cat("Estimated c by Weighted NCE-HMM:\n")
print(fit_hmm$c)

cat("Estimated c by Weighted NCE-mixture:\n")
print(fit_mix$c)

############################################################
# 13. Visualization
############################################################

par(mfrow = c(2, 2))

plot(
  z_true,
  type = "s",
  main = "True hidden states",
  xlab = "t",
  ylab = expression(z[t])
)

plot(
  fit_hmm$gamma[, 1],
  type = "l",
  ylim = c(0, 1),
  main = "Weighted NCE-HMM: P(state 1 | x_1:T)",
  xlab = "t",
  ylab = expression(gamma[t](1))
)

plot(
  fit_mix$resp[, 1],
  type = "l",
  ylim = c(0, 1),
  main = "Weighted NCE-mixture: P(state 1 | x_t)",
  xlab = "t",
  ylab = "responsibility"
)

plot(
  x,
  type = "l",
  main = "Observed time series",
  xlab = "t",
  ylab = expression(x[t])
)
